...
def app_module():
    ...
    # But: streamlit's caching decorators (st.cache_data, st.cache_resource) wrap
    # decorated functions. If we let MagicMock handle them, the decorator turns
    # `analyse_prompt` into a Mock object and tests that call it break. So we
    # explicitly install transparent decorators that just return the function
    # unchanged — the same observable behaviour real streamlit gives for our purposes.
    ...
    def _passthrough_decorator(*dargs, **dkwargs):
        ...
        if dargs and callable(dargs[0]) and not dkwargs:
            return dargs[0]
        return lambda fn: fn
    streamlit_stub.cache_data = _passthrough_decorator
    streamlit_stub.cache_resource = _passthrough_decorator
    sys.modules["streamlit"] = streamlit_stub

    ns = {"__name__": "app_under_test", "__file__": str(app_path)}
