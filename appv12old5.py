...
with col_c:
    if not selected_models:
        ...
        st.caption(f"📡 {len(selected_models)} model(s) will be called · Aggregator mode")
    else:
...
if broadcast_clicked and user_prompt.strip():
    ...
    if is_arena:
        ...
        with st.spinner("🧠 Aggregation Engine: merging outputs…"):
            synthesis = aggregate_responses(user_prompt, results, preferred_aggregator=preferred_agg)
