...
if st.session_state.query_history:
    ...
    st.caption(f"{len(st.session_state.query_history)} query/queries this session")

    for i, entry in enumerate(reversed(st.session_state.query_history)):
        idx = len(st.session_state.query_history) - i
        success_count = len(entry["successful"])
        fail_count = len(entry["failed"])
        status = f"✅ {success_count}" + (f"  ❌ {fail_count}" if fail_count else "")
        summary = entry.get("summary", "Query")
        header = f"**#{idx}** · _{summary}_ · {entry['timestamp']} · {entry['mode']} · Quality {entry['quality']}/100 · {status}"

        with st.expander(header, expanded=(i == 0)):
            st.markdown("**Prompt:**")
            st.code(entry["full_prompt"], language="text")
            mc1, mc2, mc3, mc4 = st.columns(4)
            mc1.metric("Models Used", len(entry["models_used"]))
            mc2.metric("Successful", success_count)
            fastest_name = min(entry["times"], key=entry["times"].get) if entry["times"] else "—"
            fastest_time = entry["times"].get(fastest_name, 0)
            mc3.metric("Fastest", f"{fastest_time}s", fastest_name)
            mc4.metric("Quality", f"{entry['quality']}/100")
            if entry["responses"]:
                for model_name, response_text in entry["responses"].items():
                    cfg = MODELS.get(model_name, {})
                    icon = cfg.get("icon", "•")
                    t = entry["times"].get(model_name, "?")
                    with st.expander(f"{icon} {model_name} — {t}s", expanded=False):
                        st.markdown(response_text)
            if entry["synthesis"]:
                st.markdown("**📌 Synthesised Answer:**")
                st.markdown(entry["synthesis"])
            if st.button(f"♻️ Re-use this prompt", key=f"reuse_{idx}"):
                st.session_state._prompt_to_apply = entry["full_prompt"]
                st.session_state.last_analysis = None
