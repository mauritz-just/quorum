...
class PromptAnalyzer:
    ...
    def _score_output_definition(self):
        ...
        has_structure = any(h in self.lower for h in ["json", "yaml", "xml", "table", "bullet", "markdown"])
    ...
    def _score_specificity(self):
        ...
        proper_nouns = re.findall(r"\b[A-Z][a-zA-Z]{2,}\b", self.text)
        score = 20
    ...
    def _score_cot(self, complexity):
    ...
    def _compute_complexity(self):
        has_code = any(kw in self.lower for kw in ["code", "function", "script", "program", "debug", "python", "javascript", "sql", "html", "css", "api"])
        has_creative = any(kw in self.lower for kw in ["write", "story", "poem", "essay", "blog", "creative", "imagine", "describe", "narrative"])
        has_analysis = any(kw in self.lower for kw in ["compare", "analyse", "analyze", "explain", "evaluate", "pros and cons", "difference", "versus", "vs"])
        complexity = 10
        complexity += min(self.word_count * 1.5, 40)
        complexity += 15 if has_code else 0
        complexity += 10 if has_analysis else 0
        complexity += 5 if has_creative else 0
        complexity += 10 if self.sentence_count > 3 else 0
        complexity = min(int(complexity), 100)
        return complexity, has_code, has_creative, has_analysis
    ...
    # ── Public entry point ──
    def analyse(self):
        complexity, has_code, has_creative, has_analysis = self._compute_complexity()
        if complexity < 30:
            level = "Simple"
        elif complexity < 60:
            level = "Moderate"
            ...
            level = "Complex"
        dims = {
            "Explicit Direction": self._score_direction(),
            "Defined Output": self._score_output_definition(),
            "Context & Motivation": self._score_context(),
            "Specificity": self._score_specificity(),
            "Uncertainty Safeguard": self._score_uncertainty(),
            "Chain of Thought": self._score_cot(complexity),
            "Positive Framing": self._score_framing(),
        }
        quality = int(sum(dims[k]["score"] * w for k, w in self.DIMENSION_WEIGHTS.items()))
        category = self._categorise(has_code, has_creative, has_analysis)
